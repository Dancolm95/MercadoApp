...
namespace MercadoApp.Controllers
{
    ...
    public class ReportesController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Deudas(string fechaInicio, string fechaFin)
        {
            var inicio = DateTime.Parse(fechaInicio);
            var fin = DateTime.Parse(fechaFin).AddDays(1).AddSeconds(-1);

            var deudas = await _deudaRepository.GetByFechaAsync(inicio, fin);
            var deudasList = deudas.ToList();

            var model = new ReporteDeudaViewModel
            {
                Deudas = deudasList,
                Cantidad = deudasList.Count,
                TotalMonto = deudasList.Sum(d => d.Monto)
            };
        ...
        [HttpPost]
        public async Task<IActionResult> Pagos(string fechaInicio, string fechaFin)
        {
            var inicio = DateTime.Parse(fechaInicio);
            var fin = DateTime.Parse(fechaFin).AddDays(1).AddSeconds(-1);

            var pagos = await _pagoRepository.GetByFechaAsync(inicio, fin);
            ...
            var model = new ReportePagoViewModel
            {
                ...
                TotalIngresos = pagosList.Sum(p => p.MontoPagado)
            };
        ...
        [HttpPost]
        public async Task<IActionResult> Puestos(string estado)
        {
            var puestos = await _puestoRepository.GetAllAsync();
            var puestosList = puestos.ToList();

            if (!string.IsNullOrEmpty(estado) && estado != "Todos")
            {
                puestosList = puestosList.FindAll(p => p.Estado == estado);
            }

            var model = new ReportePuestoViewModel
            {
                Puestos = puestosList,
                Cantidad = puestosList.Count
            };
        ...
        [HttpGet]
        public async Task<IActionResult> ExportarDeudasExcel(string fechaInicio, string fechaFin)
        {
            var inicio = DateTime.Parse(fechaInicio);
            var fin = DateTime.Parse(fechaFin).AddDays(1).AddSeconds(-1);
            var deudas = await _deudaRepository.GetByFechaAsync(inicio, fin);
        ...
        [HttpGet]
        public async Task<IActionResult> ExportarDeudasPdf(string fechaInicio, string fechaFin)
        {
            var inicio = DateTime.Parse(fechaInicio);
            var fin = DateTime.Parse(fechaFin).AddDays(1).AddSeconds(-1);
            var deudas = await _deudaRepository.GetByFechaAsync(inicio, fin);
        ...
        [HttpGet]
        public async Task<IActionResult> ExportarPagosExcel(string fechaInicio, string fechaFin)
        {
            var inicio = DateTime.Parse(fechaInicio);
            var fin = DateTime.Parse(fechaFin).AddDays(1).AddSeconds(-1);
            var pagos = await _pagoRepository.GetByFechaAsync(inicio, fin);
        ...
        [HttpGet]
        public async Task<IActionResult> ExportarPagosPdf(string fechaInicio, string fechaFin)
        {
            var inicio = DateTime.Parse(fechaInicio);
            var fin = DateTime.Parse(fechaFin).AddDays(1).AddSeconds(-1);
            var pagos = await _pagoRepository.GetByFechaAsync(inicio, fin);
        ...
        [HttpGet]
        public async Task<IActionResult> ExportarPuestosExcel(string estado)
        {
            var puestos = await _puestoRepository.GetAllAsync();
            var puestosList = puestos.ToList();
        ...
        [HttpGet]
        public async Task<IActionResult> ExportarPuestosPdf(string estado)
        {
            var puestos = await _puestoRepository.GetAllAsync();
            var puestosList = puestos.ToList();
